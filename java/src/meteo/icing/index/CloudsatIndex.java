...
public class CloudsatIndex 
{
	private Conf conf;
	
	private LongMap <List<String>> indexMap;
	
	public CloudsatIndex( Conf conf)
	{
		this.conf = conf;
	...
	private LongMap<List<String>> readIndex() 
	{
		...
		File rootDir = new File( conf.satDir ); 
		...
		for(File monthdir : rootDir.listFiles())
		{
			...
			for( File daydir : monthdir.listFiles())
			{
				...
				for( String filename : daydir.list())
				{
					...
					DateTime datetime = DataStamp.cloudsatFormat.parseDateTime( datePart );
