...
public class API {
	...
	private JSONObject call(String method, String what, Object payload)
		...
		while (true) {
			try {
				return this._call(method, what, payload);
	...
	private long _transfer(String from, String target, long size)
		...
		logger.log("Tranfering " + this.bytename(size) + " into " + target);
		logger.log("From " + url);
		...
		if (delta > 0) {
			...
			logger.log("Transfer rate " + this.bytename(total / elapsed) + "/s");
		}
		return total;
	}
