...
archive_folder_root = 'E:/Development/workspaces/meteo/ecmwf-api-client/download'
nc_f = archive_folder_root + '/2016-03/01/' + '2016-03-01_00Z_isobaric.nc'
...
testvar = nc_fid.variables['t']
air = testvar[:,0,:]  # shape is time, lat, lon as shown above
...
dt_time = [dt.date(1, 1, 1) + dt.timedelta(hours=int(t)) - offset\
           for t in time]
...
cbar.set_label("%s (%s)" % (testvar.var_desc,\
                            testvar.units))
plt.title("%s on %s" % (testvar.var_desc, cur_time))
