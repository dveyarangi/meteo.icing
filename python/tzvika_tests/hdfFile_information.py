...
hdfFile = 'D:/DB/Cloudsat/2006/2006-06/15/2006166131201_00702_CS_2B-CWC-RVOD_GRANULE_P_R04_E00.hdf'
...
try:  # Catch pyhdf.SD errors
    ...
    printf("-------------")
    eol()
    printf("%-25s%s" % ("File:", hdfFile))
    eol()
    printf("%-25s%d" % ("  file attributes:", len(attr)))
    eol()
    printf("%-25s%d" % ("  datasets:", len(dsets)))
    eol()
    eol()
    ...
    # Dataset table
    if len(dsets) > 0:
        printf("Datasets (idx:index num, na:n attributes, cv:coord var)")
        eol(2)
        printf("  name                 idx type    na cv dimension(s)")
        eol()
        printf("  -------------------- --- ------- -- -- ------------")
        eol()
        ...
        for name in dsNames:
            ...
            if len(dsAttr) > 0:
                ...
                for nm in attNames:
                    ...
                    # t[3] is the attribute length
                    printf("  %-20s %3d %-7s %3d %s" %
                           (nm, t[1], typeTab[t[2]], t[3], t[0]))
                    eol()
                eol()
            # Get dataset dimension dictionnary
            dsDim = dsObj.dimensions(full=1)
            if len(dsDim) > 0:
                printf("%s dimensions" % name)
                eol(2)
                printf("  name                 idx len   unl type    natt")
                eol()
                printf("  -------------------- --- ----- --- ------- ----")
                eol()
